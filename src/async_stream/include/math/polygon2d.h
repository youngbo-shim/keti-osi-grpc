...
class Polygon2d {
 public:
  ...
  /**
   * @brief Compute the shortest distance from a point on the polygon
   *        to a point in 2-D, and get the nearest point on the polygon.
   * @param point The point to compute the distance to.
   * @param nearest_pt The nearest point on the polygon
   *        to the input point.
   * @return The shortest distance from points on the polygon
   *         to the input point.
   */
  double DistanceToBoundary(const Vec2d &point, Vec2d *const nearest_pt) const;
  ...
  static bool PolygonComparator(const Polygon2d &polygon1,
                                const Polygon2d &polygon2) {
    // return p.s() < s;
    if(polygon1.min_x() == polygon2.min_x())
      return polygon1.min_y() < polygon2.min_y();
    else
      return polygon1.min_x() < polygon2.min_x();

  }

  Vec2d GetMeanPoint() const { return mean_point_; }
 ...
 protected:
  ...
  double min_y_ = 0.0;
  double max_y_ = 0.0;
  double mean_point_x_ = 0.0;
  double mean_point_y_ = 0.0;
  Vec2d mean_point_;
